...
namespace FactoryFramework
{
    ...
    public struct ArcPathStruct
    {
        public float radius;
        public float angle;
        public Vector3 center, start, end, normal, forward;
        ...
        public void Initialize(Vector3 c, Vector3 s, Vector3 e, float rad, Vector3 forward, Vector3 norm)
        {
            ...
            //Debug.Log($"generating arc path with {start} {end} {forward}");
        ...
        public Vector3 GetWorldPointFromPathSpace(float pathPercent)
        {
            ...
            Vector3 slerped = ((startdir * Mathf.Cos(theta)) + (forward * Mathf.Sin(theta)));
        ...
        private static float CalculateArcAngFromPoints(Vector3 center, Vector3 forward, Vector3 p1, Vector3 p2, Vector3 normal)
        {
            ...
            // - cross = left arc, + cross = right arc
            float cross = Vector3.Dot(Vector3.Cross(forward, (center - p1).normalized), normal);
            float rightTurn = Mathf.Sign(cross);
            float ang = Mathf.Abs(Vector3.Angle(v1, v2) * Mathf.Deg2Rad);
        ...
        public Vector3 GetDirectionAtPoint(float pathPercent)
        {
            ...
            Vector3 normalizedDir = (tanPoint-center).normalized;
            Vector3 outvec = Vector3.Cross(normalizedDir, normal) * -1 * Mathf.Sign(angle);
            //Debug.Log($"Out: {outvec}\t Angle: {angle}\t Cross: {Vector3.Cross((tanPoint-center.normalized), normal)}");
            return outvec;
        }
