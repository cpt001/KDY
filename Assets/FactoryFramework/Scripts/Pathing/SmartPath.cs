...
namespace FactoryFramework
{
    ...
    public class SmartPath : IPath, IPathMeshGenerator
    {
        ...
        public static List<IPath> TrySolve(PathAnchor start, PathAnchor end, float turnRadius, bool useStartLeft, bool useEndLeft)
        {
            ...
            Vector3 secondArcDir = (tan2 - tan1);
            secondArcDir.y = 0;
            secondArcDir.Normalize();
            if (secondArcDir == Vector3.zero)
                secondArcDir = -1 * start.forward;
            ArcPath endArc = new ArcPath(c2, tan2, end.pos, turnRadius, secondArcDir, Vector3.up);
        ...
        public static List<IPath> TryVerticalSolve(Vector3 start, Vector3 end, float rampRadius)
        {
            ...
            forward.y = 0;
            forward.Normalize();
            float angleBetween = Vector3.SignedAngle(forward, Vector3.forward, Vector3.up);
            Quaternion test = Quaternion.AngleAxis(angleBetween, Vector3.up);
            ...
            Vector3 endDir = (transtan2 - transtan1).normalized;

            return new List<IPath>() {
                new ArcPath(c1, start, transtan1, rampRadius, startDir, normal),
                new SegmentPath(transtan1, transtan2),
                new ArcPath(c2, transtan2, end, rampRadius, endDir, normal),
            };
            /*
