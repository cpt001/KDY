...
{
    [Test]
    public void RoundedPolygonStraightPath()
    {
        Vector3[] Points = {
            new Vector3(0, 0, 0),
            new Vector3(0, 0, 1),
        };
        RoundedPolygon p = new RoundedPolygon(Points, 1f,1);
        Assert.True(p.TotalLength == 1f);
        Assert.True((Vector3)p.GetDirectionAtPoint(0f) == Vector3.forward);
        Assert.True((Vector3)p.GetDirectionAtPoint(1f) == Vector3.forward);
    }
